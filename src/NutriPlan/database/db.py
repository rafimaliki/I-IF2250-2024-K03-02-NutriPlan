...
class DatabaseManager:
    ...
    def auto_update_daftar_riwayat(self):
        query = "SELECT id_jadwal,tanggal FROM jadwal_makanan WHERE id_jadwal NOT IN (SELECT jadwal FROM daftar_riwayat)"
        self.cursor.execute(query)
        jadwalId = self.cursor.fetchall()
        ...
        i = 0
        print(jadwalId)
        while len(jadwalId) > 0 and i < len(jadwalId):
            print(jadwalId[i][1])
            # if jadwalId[i][0]>=101:
            #     print(int(jadwalId[i][1][0:2]),int(jadwalId[i][1][3:5]),int(jadwalId[i][1][6:10]))
            if ((int(jadwalId[i][1][0:2]) >= int(day) and int(jadwalId[i][1][3:5]) >= int(month) and int(jadwalId[i][1][6:10])>=int(year)) or jadwalId[i][0]<101):
                jadwalId.pop(i)
                i -= 1
            i += 1
        print(jadwalId)
        if len(jadwalId)>0:
            for i in jadwalId:
                self.insert_daftar_riwayat(i[0],"")
    ...
    def update_detail_resep(self, id_resep, new_details):
        try:
            # Print debug information
            print("Updating detail_resep...")
            print("id_resep:", id_resep)
            print("New details:", new_details)
            
            # Delete existing details for the given id_resep
            delete_query = "DELETE FROM detail_resep WHERE id_resep = ?"
            self.cursor.execute(delete_query, (id_resep,))
            ...
            for detail in new_details:
                if detail != []:       
                    self.insert_detail_resep(id_resep,self.get_id_from_bahan(detail[0]),detail[1])
                    
            
            self.conn.commit()
            print(f"Updated detail_resep for id_resep = {id_resep}")
        except sqlite3.Error as e:
            print(f"Error updating detail_resep for id_resep = {id_resep}: {e}")
            self.conn.rollback()
    ...
    def get_detail_resep_from_id(self, id_resep):
        query = """
        SELECT bahan_baku.nama_bahan, detail_resep.kuantitas
        FROM detail_resep
        INNER JOIN bahan_baku ON detail_resep.id_bahan = bahan_baku.id_bahan
        WHERE detail_resep.id_resep = ?
        """
        self.cursor.execute(query, (id_resep,))
        return self.cursor.fetchall()
    ...
    def delete_jadwal(self, id_jadwal):
        ...
        self.delete_element_by_id("daftar_riwayat", "jadwal", id_jadwal)
    ...
    def get_jadwal_id_by_kat_tgl(self, kategori, tanggal):
        query = "SELECT * FROM jadwal_makanan WHERE kategori = ? AND tanggal = ?"
        self.cursor.execute(query, (kategori, tanggal))
        jadwal = self.cursor.fetchone()
        jadwal_obj = Jadwal_Makanan(jadwal[0], self.get_makanan_by_id(jadwal[1]), jadwal[2], jadwal[3])
    ...
    def delete_daftar_belanja(self, id_bahan):
        query = "DELETE FROM daftar_belanja WHERE barang = ?"
        self.cursor.execute(query, (id_bahan,))
        self.conn.commit()
        #print("Menghapus daftar belanja")
